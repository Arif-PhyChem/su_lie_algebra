...
def CNN_LSTM_train(x: np.ndarray, 
    ...
    def custom_loss(y_true, y_pred):
        if method == 'pinn' and constraint == 'Yes':
            loss = ml_loss.pinn_loss(y_true, y_pred, n_states, lambda_1=1.0, lambda_2=2.0, lambda_3=3.0, lambda_4=3.0)
        if method == 'su2-pinn' and constraint == 'Yes':
            loss = ml_loss.su_loss(y_true, y_pred, basis_matrices, n_states, lambda_1=2.0, lambda_2=3.0, lambda_3=3.0)
        if method == 'punn' or method == 'su2-punn':
    ...
    models_dir =  method +  "cnn_lstm_trained_models"
...
def LSTM_train(x: np.ndarray, 
    ...
    models_dir =  method + "lstm_trained_models"
    if not os.path.exists(models_dir):
        os.makedirs(models_dir)
        print("Directory",  models_dir, "is created sucessfully where the trained models will be saved")
    else:
