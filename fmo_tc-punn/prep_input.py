...
#
    ...
    xx = all_files[file_name]
    length = xx[0:201,0].shape[0]
    print(length)
    if systemType == 'FMO':
        x = np.zeros((file_count, 4), dtype=float)
    if systemType == 'SB':
        x = np.zeros((file_count, 5), dtype=float)
    y = np.zeros((file_count, (n_states**2 - 1) * length), dtype=float)
    ...
    labels.pop(int((n_states*(n_states -1))/2) + (n_states - 1))  # delete the last diag element
...
, otherwise training files will not be successfully generated')
    ...
    for files in glob.glob(dataPath+'/*np[yz]'):
        ...
        if systemType == 'FMO':
            ...
            lamb[j] = y[1]
            y = re.split(r'-', x[4])
            x = re.split(r'_coefficients.npy', y[1]) # extract value of temperature
            temp[j] = x[0]
        if systemType == 'SB': 
            x = re.split(r'-', file_name)
            y = re.split(r'_', x[1])
            ...
            y = re.split(r'_', x[4])
            gamma[j] = y[0]
            y = re.split(r'_coefficients.npy', x[5])
            temp[j] = y[0]
        j = j + 1
#
    print('prep_input.OSTL: Normalizing gamma, lambda and temperature using the following',
...
#
    ...
    xx = all_files[file_name]
    length = 201 #xx[:,0].shape[0]
    if systemType == 'FMO':
        x = np.zeros((file_count, 4), dtype=float)
    if systemType == 'SB':
        x = np.zeros((file_count, 5), dtype=float)
    y = np.zeros((file_count, (n_states**2-1) * length), dtype=float)
    yy = np.zeros((file_count, (n_states**2-1)), dtype=float)
    ...
    for files in glob.glob(dataPath+'/*np[yz]'):
        ...
        q = 0
        for i in range(0, length):
            yy[0,:] = df[i, 1:n_states**2].real # excluding the 1st column of time
            
            y[m,q:q+(n_states**2 -1)] = yy[0, :]
...
#
    ...
    for i in t:
        ...
        for j in range(0, numLogf): 
            ...
            c += 5.0
        u += 1
    y = np.zeros((file_count*length, n_states+(n_states-1), n_states+(n_states-1)), dtype=float)
    yy = np.zeros((1, n_states**2), dtype=complex)
    labels = np.zeros((1, int((n_states**2-n_states)/2 + n_states)), dtype = int)
    labels = []
    m = 0; 
